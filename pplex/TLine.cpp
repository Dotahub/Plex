...
TLine::TLine(TPoint * left, TPoint * right) : TBase(LINE, left->getName() + " " + right->getName(), 10)
{
	this->left = left;
	this->right = right;
}

TLine::TLine(const TLine & src) : TBase(LINE, src.left->getName() + " " + src.right->getName(), 10)
{
	this->left = src.left;
...
void TLine::show(System::Drawing::Graphics ^ g)
{
	if (isVisible == 1)
	{
		...
		//gcnew System::Drawing::Pen(ColorForPoint, (float)thickness)
